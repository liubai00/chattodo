...
/**
 * 一键启动脚本：清理 PGlite 残留 PID 文件 → 同时启动前后端（新栈 backend/apps/api）
 *
 * 使用方法：node scripts/dev.mjs
 * 或通过 npm：npm run dev:all（根目录）
 */
...
// ── 1. 清理 PGlite 残留 postmaster.pid / postmaster.opts ─────────────
// （上次强杀可能残留，卡住下次启动）
const pgdataDirs = [join(ROOT, 'data', 'pgdata-dev')]
...
for (const dir of pgdataDirs) {
  for (const name of ['postmaster.pid', 'postmaster.opts']) {
    ...
    if (existsSync(f)) {
      try {
        unlinkSync(f)
        console.log(`[clean] 已删除残留: ${f}`)
      } catch (err) {
        console.warn(`[clean] 无法删除 ${f}: ${err.message}`)
      }
    }
  }
}

// ── 2. 启动后端（新栈 apps/api，PGlite 零配置模式）────────────────────
// 直接用 node 启动（不经 npm 包装层），stdio: 'inherit' 让终端 Ctrl+C 的 SIGINT
// 直达后端进程，触发 main.ts 的优雅关闭：app.close() → db.close()（PGlite 落盘）。
console.log('[api] 启动后端 (port 18787, PGlite) …')
const server = spawn(process.execPath, ['--env-file-if-exists=.env', join(ROOT, 'scripts', 'dev-api.mjs')], {
  cwd: ROOT,
  stdio: 'inherit',
})
...
// ── 4. 优雅退出 ──────────────────────────────────────────────────
// 关键：必须给后端时间走 main.ts 的优雅关闭（app.close → db.close 落盘）。
// 若 kill 后立即 process.exit，后端被强杀 -> PGlite 半写入 -> 下次启动可能损坏。
...
async function cleanup(signal) {
  ...
  // 后端：终端 Ctrl+C 已通过 stdio:inherit 直达后端，main.ts 的 graceful shutdown
  // 正在自行执行。Windows 上 server.kill('SIGINT') 实为 TerminateProcess（强杀），
  // 会打断 PGlite 落盘导致数据目录损坏——故不主动发信号，只等其自行退出，超时再兜底。
