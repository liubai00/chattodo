...
<template>
  <div class="flex h-full min-h-0">
    <!-- dbViews 导航列 -->
    <div class="flex flex-col border-r border-[var(--line)] bg-[var(--panel)]" style="width:200px;flex:0 0 200px;">
    ...
    <div class="flex flex-1 flex-col">
      ...
      <!-- 筛选栏 -->
      <div class="flex h-[52px] flex-none items-center gap-2.5 border-b border-[var(--line)] bg-[var(--panel)] px-[18px]">
        <div class="flex items-center gap-2 rounded-[9px] bg-[var(--mid)] px-[11px] py-[7px]" style="width:230px;">
      ...
      <template v-if="dbLayout === 'table'">
        <div class="flex-1 overflow-auto">
          <div class="grid border-b border-[var(--line)] bg-[var(--bg)]" style="grid-template-columns:36px 1fr 112px 100px 76px 88px 60px;gap:0;padding:0 22px;position:sticky;top:0;z-index:1;">
          ...
          <div v-for="t in filteredTasks" :key="t.id" :style="`display:grid;grid-template-columns:36px 1fr 112px 100px 76px 88px 60px;gap:0;padding:0 22px;border-bottom:1px solid var(--line);align-items:center;background:${t.rowBg};`" data-hv="0">
      ...
      <template v-if="dbLayout === 'board'">
        <div class="flex flex-1 items-stretch gap-4 overflow-auto p-[18px]">
          <div v-for="col in boardCols" :key="col.key" @drop="col.onDrop" @dragover="col.onOver" @dragleave="col.onLeave" :style="`flex:1;min-width:0;background:var(--panel);border:1px solid ${col.hl?'var(--accent)':'var(--line)'};border-radius:14px;display:flex;flex-direction:column;overflow:hidden;transition:border-color .12s;${col.hl?'box-shadow:0 0 0 2px var(--accent-bg);':''}`">
