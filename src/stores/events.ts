// Phase 2：实时事件的单一 SSE 归属（Pinia store）。
// 全局只此一处调用 api.subscribeEvents 开 /api/events 连接；旧 App.vue 的 _startEvents
...
export const useEventsStore = defineStore('events', () => {
  ...
  // 注：api.subscribeEvents 内部对断线静默重连(5s)、错误不外抛，故无 'error' 态。
