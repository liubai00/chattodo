...
// P14: 内联 animation:lx-fade 替换为 v-message-enter 指令（150ms opacity 0→1 + y+4px，仅 mount 触发；streaming 跳过）。
  ...
  <div id="lx-msgs" class="flex flex-1 flex-col gap-[17px] overflow-auto p-[26px]">
    ...
    <template v-else>
      <template v-for="m in messages" :key="m.id">
        ...
        <div v-else-if="m.isUser" v-message-enter class="flex flex-col items-end gap-[5px] self-end" style="max-width:78%;">
        ...
        <div v-else-if="m.isAgentText" v-message-enter="m.streaming" class="flex gap-[9px] self-start" style="max-width:82%;">
        ...
        <div v-else-if="m.isTask" v-message-enter class="flex flex-col gap-2 self-start" style="max-width:82%;">
        ...
        <div v-else-if="m.isIdea" v-message-enter class="flex flex-col gap-2 self-start" style="max-width:82%;">
        ...
        <div v-else-if="m.isNono" v-message-enter class="flex flex-col gap-2 self-start" style="max-width:82%;">
        ...
        <div v-else-if="m.isPlan" v-message-enter class="self-start rounded-[var(--r)] border border-[var(--line)] bg-[var(--panel)] p-[15px] shadow-md" style="max-width:82%;">
        ...
        <div v-else-if="m.isError" v-message-enter class="flex flex-col gap-[9px] self-start rounded-[var(--r)] border-l-[3px] border-[var(--danger)] bg-[var(--danger-bg)] p-[13px_15px]" style="max-width:82%;">
