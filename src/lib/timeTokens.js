...
export function expandTimeTokens(text, { now = new Date(), timeZone } = {}) {
  ...
  const timeStr = () => `${pad(p.h)}:${pad(p.mi)}`
  const render = (r) => (r.kind === 'datetime' ? `${dateStr(0)} ${timeStr()}` : r.kind === 'time' ? timeStr() : dateStr(r.off))

  return s.replace(/@([A-Za-z]+|[一-鿿]+)/g, (m, word) => {
    if (/^[A-Za-z]/.test(word)) { const r = MAP[word.toLowerCase()]; return r ? render(r) : m } // 英文整段精确匹配（大小写不敏感）
    for (const a of CJK) if (word.startsWith(a)) return render(MAP[a]) + word.slice(a.length) // 中文取最长别名前缀
    return m
  })
}
