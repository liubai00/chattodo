...
test('chat plan intent: returns plan + persists user & agent messages', async () => {
  ...
  assert.equal((await db.prepare('SELECT COUNT(*) AS c FROM chat_messages').get()).c, 3) // welcome + user + agent
})
...
test('chat capture intent: triages, creates entity, replies', async () => {
  ...
  assert.ok((await db.prepare('SELECT COUNT(*) AS c FROM tasks').get()).c >= 6)
})
