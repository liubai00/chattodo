...
test('idea archive + discard', async () => {
  ...
  assert.equal(db.prepare("SELECT COUNT(*) AS c FROM todo_ideas WHERE id='idea_research'").get().c, 0)
})
...
test('non-todo convert-to-todo creates task + removes non', async () => {
  ...
  assert.equal(db.prepare("SELECT COUNT(*) AS c FROM non_todo_outputs WHERE id='non_cubox'").get().c, 0)
})

test('non-todo discard', async () => {
  const { app, db } = makeTestApp()
  await app.inject({ method: 'POST', url: '/api/non-todo-outputs/non_quote/discard' })
  assert.equal(db.prepare("SELECT COUNT(*) AS c FROM non_todo_outputs WHERE id='non_quote'").get().c, 0)
})
