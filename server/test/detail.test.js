...
test('add + toggle subtask (logs activity)', async () => {
  ...
  assert.ok((await db.prepare("SELECT COUNT(*) AS c FROM activity WHERE task_id='task_api'").get()).c >= 1)
})
...
test('assign via PATCH persists + logs activity', async () => {
  ...
  assert.equal((await db.prepare("SELECT assignee FROM tasks WHERE id='task_api'").get()).assignee, '王敏')
  assert.ok((await db.prepare("SELECT COUNT(*) AS c FROM activity WHERE task_id='task_api' AND text LIKE '指派%'").get()).c >= 1)
})

test('delete task', async () => {
  const { app, db } = await makeTestApp()
  await app.inject({ method: 'DELETE', url: '/api/tasks/task_review' })
  assert.equal((await db.prepare("SELECT COUNT(*) AS c FROM tasks WHERE id='task_review'").get()).c, 0)
})
...
test('capture task logs creation activity', async () => {
  ...
  assert.ok((await db.prepare('SELECT COUNT(*) AS c FROM activity WHERE task_id=?').get(cap.entity.id)).c >= 1)
})
