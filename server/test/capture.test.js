...
test('POST /api/capture → task: persists + writes capture_record', async () => {
  ...
  const row = await db.prepare('SELECT * FROM tasks WHERE id = ?').get(body.entity.id)
  assert.ok(row, 'task persisted')

  const rec = await db.prepare('SELECT * FROM capture_records WHERE result_entity_id = ?').get(body.entity.id)
