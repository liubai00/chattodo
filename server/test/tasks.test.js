...
test('move-out: task → non_todo (corrected) + correction + relinked record', async () => {
  ...
  assert.equal((await db.prepare('SELECT COUNT(*) AS c FROM tasks WHERE id=?').get(taskId)).c, 0)
  assert.ok((await db.prepare('SELECT COUNT(*) AS c FROM corrections').get()).c >= 1)
  const rec = await db.prepare('SELECT * FROM capture_records WHERE result_entity_id=?').get(res.nonTodo.id)
  assert.ok(rec)
