...
export default async function adminRoutes(app) {
  ...
  app.get('/api/admin/overview', async (req, reply) => {
    ...
    const [users, tasks, ideas, nons, errs, totalErrors] = await Promise.all([
      db.all(`SELECT id, name, email, role, created_at FROM users ORDER BY created_at`),
  ...
  app.get('/api/admin/users/:id', async (req, reply) => {
    ...
    const user = await db.get(`SELECT id, name, email, role, created_at FROM users WHERE id = ?`, [uid])
