...
export default async function collabRoutes(app) {
  app.post('/api/tasks/:id/invite', async (req, reply) => {
    const r = inviteFx(app.db, req.repos, req.user, req.params.id, String((req.body && req.body.userId) || ''), { force: !!(req.body && req.body.force) })
  ...
  app.get('/api/invites', async (req) => ({ invites: req.repos.collaborators.myPending() }))
  ...
  app.post('/api/invites/:id/respond', async (req, reply) => {
    ...
    const r = respondInviteFx(app.db, req.repos, req.user, req.params.id, mode, remind)
    if (!r) return reply.status(404).send({ error: '邀请不存在或已处理' })
    return r
  })

  // 自动化规则管理（记忆驱动的自动邀请）
  app.get('/api/auto-rules', async (req) => ({ rules: req.repos.autoRules.all() }))
  app.delete('/api/auto-rules/:id', async (req) => { req.repos.autoRules.remove(req.params.id); return { ok: true } })

  app.post('/api/tasks/:id/leave', async (req, reply) => {
    const repos = req.repos
    const task = repos.tasks.get(req.params.id)
    if (!task) return reply.status(404).send({ error: 'task not found' })
    if (!repos.collaborators.leave(req.params.id)) return reply.status(400).send({ error: '你不是该任务的协作者' })
    const meName = (req.user && req.user.name) || '协作者'
    const ownerId = app.db.prepare(`SELECT user_id FROM tasks WHERE id = ?`).get(req.params.id).user_id
    app.db.prepare(`INSERT INTO notifications (id,user_id,type,icon,color,text,read,created_at) VALUES (?,?,?,?,?,?,0,?)`)
      .run('nt_' + Math.random().toString(36).slice(2, 10), ownerId, 'assign', 'ph-sign-out', 'var(--text3)', `${meName} 退出了「${task.title}」的协作`, new Date().toISOString())
    return { ok: true }
  })
}
