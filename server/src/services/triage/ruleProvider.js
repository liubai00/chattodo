...
export function detectDue(text) {
  for (const [hint, days] of TIME_HINTS) {
    if (text.includes(hint) && days !== null) return isoDaysFromNow(days)
  }
...
// Chat intent: plan question vs capture (ported from triage.js).
export function detectIntent(message) {
  const m = message.trim()
  const planMarkers = ['做什么', '接下来', '计划', '安排什么', '该干嘛', '下一步做', '两小时']
  return planMarkers.some((k) => m.includes(k)) ? 'plan' : 'capture'
}
